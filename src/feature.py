...
def feature_engineering(df):
    ...
    df['無形固定資産変動ソフトウェア関連/売上'] = df['無形固定資産変動(ソフトウェア関連)'] / (df['売上'] + epsilon)
    df['sqrt(店舗数)/事業所数'] = np.sqrt(df['店舗数']) / (df['事業所数'] + epsilon)
    df['固定資産/売上'] = df['固定資産'] / (df['売上'] + epsilon)
    df['営業CF/総資産'] = df['営業CF'] / (df['総資産'] + epsilon)
    df['営業利益/総資産'] = df['営業利益'] / (df['総資産'] + epsilon)
...
def survey_features(df, question_cols,
                    pos_q_cols=['アンケート１', 'アンケート２', 'アンケート３',
                                'アンケート５', 'アンケート８', 'アンケート９', 'アンケート１１'],
                    neg_q_cols=['アンケート４']):
    # ポジネガ
...
def transformer_embedding(df, model_name='intfloat/multilingual-e5-base', normalize_embeddings=False):
    ...
    df_embeddings.columns = df.name + "_emb_" + \
        df_embeddings.columns.astype(str)
